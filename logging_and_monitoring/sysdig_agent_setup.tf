...
resource kubernetes_cluster_role sysdig_agent {
  ...
  depends_on = [
    ibm_resource_key.sysdig_secret,
...
resource kubernetes_config_map sysdig_config_map {
  ...
  data = {
    ...
    collector: "ingest.private.${var.ibm_region}.monitoring.cloud.ibm.com"
    collector_port: 6443
    ssl: true
...
resource kubernetes_daemonset sysdig_agent {
  ...
  depends_on = [
    kubernetes_config_map.sysdig_config_map
  ]
}
